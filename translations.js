var linguJSON = {
  languages: [
    {
      lang_name: "PT",
      lang_code: "pt",
      url_pattern: "?",
    },
    {
      lang_name: "EN",
      lang_code: "en",
      url_pattern: "?",
    },
    {
      lang_name: "FR",
      lang_code: "fr",
      url_pattern: "?",
    },
    {
      lang_name: "ES",
      lang_code: "es",
      url_pattern: "?",
    },
  ],
  custom_lang_switcher_html:
    '<div id="langswitcher" class="notranslate"><a id="currlanglink" class="language-item" href="">[[linguCurrLang]]</a><div class="divider-languages"></div><a href="[[linguTargetLangHref1]]" class="targetlang language-item">[[linguTargetLang1]]</a><div class="divider-languages"></div><a href="[[linguTargetLangHref2]]" class="targetlang language-item">[[linguTargetLang2]]</a><div class="divider-languages"></div><a href="[[linguTargetLangHref3]]" class="targetlang language-item">[[linguTargetLang3]]</a></div>',
  custom_lang_switcher_container_id: "langlinks",

  translated_segments: [
    {
      source: "Institucional",
      target_en: "Institutional",
      target_fr: "Institutionnel",
      target_es: "Institucional",
    },
    {
      source: "Serviços",
      target_en: "Services",
      target_fr: "Services",
      target_es: "Servicios",
    },
    {
      source: "Design, Protótipos e Pequenas Séries",
      target_en: "Design, Prototypes and Small Series",
      target_fr: "Design, Prototypes et Petites Séries",
      target_es: "Diseño, Prototipos y Pequeñas Series",
    },
    {
      source: "Produção de Moldes e Validação",
      target_en: "Mold Production and Validation",
      target_fr: "Production de Moules et Validation",
      target_es: "Producción de Moldes y Validación",
    },
    {
      source: "Injeção de Termoplásticos e Assemblagem",
      target_en: "Thermoplastic Injection and Assembly",
      target_fr: "Injection de Thermoplastiques et Assemblage",
      target_es: "Inyección de Termoplásticos y Montaje",
    },
    {
      source: "Industrias | Casos de Estudo",
      target_en: "Industries | Case Studies",
      target_fr: "Industries | Études de Cas",
      target_es: "Industrias | Casos de Estudio",
    },
    {
      source: "Notícias",
      target_en: "News",
      target_fr: "Actualités",
      target_es: "Noticias",
    },
    {
      source: "Contactos",
      target_en: "Contacts",
      target_fr: "Contacts",
      target_es: "Contactos",
    },
    {
      source: "home",
      target_en: "home",
      target_fr: "accueil",
      target_es: "inicio",
    },

    //HOME

    {
      source:
        "Fundada em 1993, a <strong>TCC</strong> é uma empresa de produção de moldes de precisão em aço especializada na <strong>ELABORAÇÃO DE PROJETOS</strong>, <strong>FABRICO</strong> e <strong>CONTROLO DE QUALIDADE</strong> de moldes de injeção para a indústria de plásticos.",
      target_en:
        "Founded in 1993, <strong>TCC</strong> is a company specializing in the production of precision steel molds, specializing in <strong>PROJECT DEVELOPMENT</strong>, <strong>MANUFACTURING</strong>, and <strong>QUALITY CONTROL</strong> of injection molds for the plastics industry.",
      target_fr:
        "Fondée en 1993, <strong>TCC</strong> est une entreprise spécialisée dans la production de moules de précision en acier, spécialisée dans l'<strong>ÉLABORATION DE PROJETS</strong>, la <strong>FABRICATION</strong> et le <strong>CONTRÔLE DE QUALITÉ</strong> de moules d'injection pour l'industrie du plastique.",
      target_es:
        "Fundada en 1993, <strong>TCC</strong> es una empresa especializada en la producción de moldes de precisión en acero, especializada en la <strong>ELABORACIÓN DE PROYECTOS</strong>, <strong>FABRICACIÓN</strong> y <strong>CONTROL DE CALIDAD</strong> de moldes de inyección para la industria del plástico.",
    },
    {
      source:
        "Situada na Marinha Grande, um dos maiores centros industriais de moldes da Europa, a <strong>TCC</strong> orgulha-se de ter como referência clientes exigentes localizados em países tais como a Espanha, França, Bélgica, Holanda, Finlândia, Suécia, Dinamarca, Brasil e México.",
      target_en:
        "Located in Marinha Grande, one of the largest industrial mold centers in Europe, <strong>TCC</strong> prides itself on serving demanding clients located in countries such as Spain, France, Belgium, Netherlands, Finland, Sweden, Denmark, Brazil, and Mexico.",
      target_fr:
        "Située à Marinha Grande, l'un des plus grands centres industriels de moules en Europe, <strong>TCC</strong> est fière de compter parmi ses clients exigeants des pays tels que l'Espagne, la France, la Belgique, les Pays-Bas, la Finlande, la Suède, le Danemark, le Brésil et le Mexique.",
      target_es:
        "Ubicada en Marinha Grande, uno de los mayores centros industriales de moldes en Europa, <strong>TCC</strong> se enorgullece de contar con clientes exigentes ubicados en países como España, Francia, Bélgica, Países Bajos, Finlandia, Suecia, Dinamarca, Brasil y México.",
    },
    {
      source:
        "Graças à nossa experiência e amplo conhecimento da indústria dos moldes conjugado com a utilização de tecnologia de ponta, podemos fornecer moldes de alta qualidade, independentemente da sua complexidade, a um preço muito competitivo.",
      target_en:
        "Thanks to our experience and extensive knowledge of the mold industry combined with the use of cutting-edge technology, we can provide high-quality molds, regardless of their complexity, at a very competitive price.",
      target_fr:
        "Grâce à notre expérience et à notre connaissance approfondie de l'industrie des moules associée à l'utilisation de technologies de pointe, nous pouvons fournir des moules de haute qualité, quelle que soit leur complexité, à un prix très compétitif.",
      target_es:
        "Gracias a nuestra experiencia y amplio conocimiento de la industria de moldes combinado con el uso de tecnología de vanguardia, podemos proporcionar moldes de alta calidad, independientemente de su complejidad, a un precio muy competitivo.",
    },
    {
      source:
        "Conscientes da agressividade do mercado e das necessidades dos nossos clientes, procuramos as melhores e mais otimizadas soluções tendo em vista a obtenção de prazos cada vez mais curtos",
      target_en:
        "Aware of the market's competitiveness and our clients' needs, we strive to find the best and most optimized solutions to achieve increasingly shorter deadlines.",
      target_es:
        "Conscientes de la agresividad del mercado y las necesidades de nuestros clientes, buscamos las mejores y más optimizadas soluciones para obtener plazos cada vez más cortos.",
      target_fr:
        "Conscients de la compétitivité du marché et des besoins de nos clients, nous nous efforçons de trouver les meilleures solutions optimisées pour atteindre des délais de plus en plus courts.",
    },
    {
      source:
        "Na TCC, cuidamos do seu projeto desde a conceção até à produção final. Os nossos engenheiros trabalham para lhe oferecer rapidamente um protótipo e a primeira produção das suas peças.",
      target_en:
        "At TCC, we take care of your project from conception to final production. Our engineers work to quickly provide you with a prototype and the initial production of your parts.",
      target_fr:
        "Chez TCC, nous nous occupons de votre projet de la conception à la production finale. Nos ingénieurs travaillent pour vous fournir rapidement un prototype et la première production de vos pièces.",
      target_es:
        "En TCC, nos ocupamos de su proyecto desde la concepción hasta la producción final. Nuestros ingenieros trabajan para ofrecerle rápidamente un prototipo y la primera producción de sus piezas.",
    },
    {
      source: "Saber mais",
      target_en: "Learn more",
      target_fr: "En savoir plus",
      target_es: "Más información",
    },
    {
      source:
        "Trabalhamos em estreita colaboração com centros de testes que nos permitem ter uma grande eficiência e reduzir o tempo antes de enviar os moldes.",
      target_en:
        "We work closely with testing centers that allow us to achieve high efficiency and reduce lead time before sending out the molds.",
      target_fr:
        "Nous travaillons en étroite collaboration avec des centres de tests qui nous permettent d'atteindre une grande efficacité et de réduire le délai avant l'envoi des moules.",
      target_es:
        "Trabajamos en estrecha colaboración con centros de pruebas que nos permiten lograr una gran eficiencia y reducir el tiempo antes de enviar los moldes.",
    },
    {
      source:
        "Dotados de certificação ISO 6 e de uma sala limpa, podemos testar moldes destinados à indústria farmacêutica ou médica. Dispomos ainda de duas fresadoras CNC novas, equipadas com software de medição (protocolo de medição BLUM) que permite inspecionar qualquer peça, permitindo ganhos significativos de produtividade e fiabilidade.",
      target_en:
        "Equipped with ISO 6 certification and a clean room, we can test molds intended for the pharmaceutical or medical industry. We also have two new CNC milling machines equipped with measurement software (BLUM measurement protocol) that allows inspection of any part, resulting in significant gains in productivity and reliability.",
      target_fr:
        "Dotés de la certification ISO 6 et d'une salle blanche, nous pouvons tester des moules destinés à l'industrie pharmaceutique ou médicale. Nous disposons également de deux nouvelles fraiseuses CNC équipées d'un logiciel de mesure (protocole de mesure BLUM) qui permet l'inspection de toute pièce, entraînant des gains significatifs de productivité et de fiabilité.",
      target_es:
        "Dotados de certificación ISO 6 y de una sala limpia, podemos probar moldes destinados a la industria farmacéutica o médica. También contamos con dos fresadoras CNC nuevas equipadas con software de medición (protocolo de medición BLUM) que permite inspeccionar cualquier pieza, lo que proporciona ganancias significativas en productividad y fiabilidad.",
    },
    {
      source: "Aeronáutica",
      target_en: "Aeronautics",
      target_fr: "Aéronautique",
      target_es: "Aeronáutica",
    },
    {
      source: "Área Automóvel",
      target_en: "Automotive Sector",
      target_fr: "Domaine Automobile",
      target_es: "Sector Automotriz",
    },
    {
      source: "Área Médica",
      target_en: "Medical Sector",
      target_fr: "Domaine Médical",
      target_es: "Sector Médico",
    },
    {
      source: "Eletro-eletrónica",
      target_en: "Electroelectronics",
      target_fr: "Électroélectronique",
      target_es: "Electroelectrónica",
    },
    {
      source: "Área Alimentar",
      target_en: "Food Sector",
      target_fr: "Domaine Alimentaire",
      target_es: "Sector Alimentario",
    },
    {
      source: "Houseware",
      target_en: "Houseware",
      target_fr: "Articles ménagers",
      target_es: "Artículos para el hogar",
    },
    {
      source: "Agricultura e Indústria",
      target_en: "Agriculture and Industry",
      target_fr: "Agriculture et Industrie",
      target_es: "Agricultura e Industria",
    },
    {
      source: "Packaging",
      target_en: "Packaging",
      target_fr: "Emballage",
      target_es: "Embalaje",
    },
    {
      source: "Indústrias que servimos",
      target_en: "Industries we serve",
      target_fr: "Industries que nous servons",
      target_es: "Industrias que servimos",
    },
    {
      source: "Feedback de clientes",
      target_en: "Customer feedback",
      target_fr: "Feedback des clients",
      target_es: "Opiniones de clientes",
    },

    //INSTITUCIONAL
    {
      source:
        "Na vanguarda da precisão, a moldar o futuro da indústria com <strong>excelência</strong>.",
      target_en:
        "At the forefront of precision, shaping the future of the industry with <strong>excellence</strong>.",
      target_fr:
        "À la pointe de la précision, façonnant l'avenir de l'industrie avec <strong>excellence</strong>.",
      target_es:
        "En la vanguardia de la precisión, dando forma al futuro de la industria con <strong>excelencia</strong>.",
    },
    {
      source: "A nossa equipa",
      target_en: "Our team",
      target_fr: "Notre équipe",
      target_es: "Nuestro equipo",
    },
    {
      source:
        "Na <strong>TCC</strong> encontra uma equipa técnica composta por projetistas experientes, que apoiados por sistemas CAD/CAM desenvolvem os projetos dos moldes, sendo também responsáveis pelo acompanhamento de todas a etapas de fabricação.",
      target_en:
        "At <strong>TCC</strong>, you will find a technical team composed of experienced designers who, supported by CAD/CAM systems, develop mold projects and are also responsible for overseeing all stages of fabrication.",
      target_es:
        "En <strong>TCC</strong>, encontrarás un equipo técnico compuesto por diseñadores experimentados que, apoyados por sistemas CAD/CAM, desarrollan proyectos de moldes y también son responsables de supervisar todas las etapas de fabricación.",
      target_fr:
        "Chez <strong>TCC</strong>, vous trouverez une équipe technique composée de concepteurs expérimentés qui, soutenus par des systèmes CAD/CAM, développent des projets de moules et sont également responsables de superviser toutes les étapes de fabrication.",
    },

    {
      source:
        "A nossa excelente organização permite-nos gerir projetos de grande envergadura sem descurar a qualidade, respeitando imperativamente os prazos de entrega.",
      target_en:
        "Our excellent organization allows us to manage large-scale projects without neglecting quality, strictly respecting delivery deadlines.",
      target_fr:
        "Notre excellente organisation nous permet de gérer des projets de grande envergure sans négliger la qualité, en respectant strictement les délais de livraison.",
      target_es:
        "Nuestra excelente organización nos permite gestionar proyectos de gran envergadura sin descuidar la calidad, respetando estrictamente los plazos de entrega.",
    },
    {
      source:
        "Os nossos comerciais mantêm um contacto estreito com os nossos clientes desde o primeiro contacto, garantindo um acompanhamento eficiente de todo o processo de fabrico do molde e fornecendo um serviço de pós-venda de qualidade.",
      target_en:
        "Our sales representatives maintain close contact with our clients from the first interaction, ensuring efficient monitoring of the entire mold manufacturing process and providing quality after-sales service.",
      target_fr:
        "Nos commerciaux maintiennent un contact étroit avec nos clients dès le premier contact, assurant un suivi efficace de tout le processus de fabrication du moule et fournissant un service après-vente de qualité.",
      target_es:
        "Nuestros comerciales mantienen un contacto estrecho con nuestros clientes desde el primer contacto, garantizando un seguimiento eficiente de todo el proceso de fabricación del molde y proporcionando un servicio postventa de calidad.",
    },
    {
      source: "Abraçar desafios com inovação, precisão e espírito de equipa:",
      target_en:
        "Embracing challenges with innovation, precision, and teamwork:",
      target_fr:
        "Relever les défis avec innovation, précision et esprit d'équipe :",
      target_es:
        "Afrontar desafíos con innovación, precisión y espíritu de equipo:",
    },
    {
      source:
        "a alma da nossa <strong>cultura</strong>, a força do nosso <strong>mindset</strong>.",
      target_en:
        "the soul of our <strong>culture</strong>, the strength of our <strong>mindset</strong>.",
      target_fr:
        "l'âme de notre <strong>culture</strong>, la force de notre <strong>mindset</strong>.",
      target_es:
        "el alma de nuestra <strong>cultura</strong>, la fuerza de nuestro <strong>mindset</strong>.",
    },
    {
      source:
        "A TCC destaca-se pela sua abordagem inovadora e orientada para a excelência, destacando-se no mercado pela precisão dos seus produtos e pelo forte espírito de equipa que impulsiona cada projeto.",
      target_en:
        "TCC stands out for its innovative and excellence-oriented approach, distinguishing itself in the market for the precision of its products and the strong teamwork spirit that drives each project.",
      target_fr:
        "TCC se distingue par son approche innovante et axée sur l'excellence, se distinguant sur le marché par la précision de ses produits et l'esprit d'équipe fort qui anime chaque projet.",
      target_es:
        "TCC se destaca por su enfoque innovador y orientado a la excelencia, destacándose en el mercado por la precisión de sus productos y por el fuerte espíritu de equipo que impulsa cada proyecto.",
    },
    {
      source:
        "Com um compromisso inabalável com a qualidade e um foco contínuo na satisfação do cliente, a TCC é a escolha ideal para aqueles que procuram soluções de moldagem de alto nível, onde a tradição de excelência encontra-se com a vanguarda da tecnologia.",
      target_en:
        "With an unwavering commitment to quality and a continuous focus on customer satisfaction, TCC is the ideal choice for those seeking high-level molding solutions, where the tradition of excellence meets cutting-edge technology.",
      target_fr:
        "Avec un engagement inébranlable envers la qualité et une attention continue à la satisfaction du client, TCC est le choix idéal pour ceux qui recherchent des solutions de moulage de haut niveau, où la tradition d'excellence rencontre la technologie de pointe.",
      target_es:
        "Con un compromiso inquebrantable con la calidad y un enfoque continuo en la satisfacción del cliente, TCC es la opción ideal para aquellos que buscan soluciones de moldeo de alto nivel, donde la tradición de excelencia se encuentra con la vanguardia de la tecnología.",
    },
    {
      source: "Inovação e Tecnologia",
      target_en: "Innovation and Technology",
      target_fr: "Innovation et Technologie",
      target_es: "Innovación y Tecnología",
    },
    {
      source:
        "A TCC valoriza a inovação e a adoção de tecnologias avançadas para melhorar a qualidade e eficiência dos produtos.",
      target_en:
        "TCC values innovation and the adoption of advanced technologies to improve the quality and efficiency of products.",
      target_fr:
        "TCC valorise l'innovation et l'adoption de technologies avancées pour améliorer la qualité et l'efficacité des produits.",
      target_es:
        "TCC valora la innovación y la adopción de tecnologías avanzadas para mejorar la calidad y eficiencia de los productos.",
    },
    {
      source: "Trabalho de equipa e Colaboração",
      target_en: "Teamwork and Collaboration",
      target_fr: "Travail d'équipe et Collaboration",
      target_es: "Trabajo en equipo y Colaboración",
    },
    {
      source:
        "Devido à natureza complexa dos projetos de fabricação de moldes, a TCC valoriza o trabalho em equipa e a colaboração entre diferentes departamentos e especialidades.",
      target_en:
        "Due to the complex nature of mold manufacturing projects, TCC values teamwork and collaboration among different departments and specialties.",
      target_fr:
        "En raison de la nature complexe des projets de fabrication de moules, TCC valorise le travail d'équipe et la collaboration entre différents départements et spécialités.",
      target_es:
        "Debido a la naturaleza compleja de los proyectos de fabricación de moldes, TCC valora el trabajo en equipo y la colaboración entre diferentes departamentos y especialidades.",
    },
    {
      source: "Compromisso com o cliente",
      target_en: "Commitment to Customer",
      target_fr: "Engagement envers le client",
      target_es: "Compromiso con el cliente",
    },
    {
      source:
        "A satisfação do cliente é essencial para nós. Na TCC procuramos entender e atender às necessidades específicas dos clientes de forma eficaz.",
      target_en:
        "Customer satisfaction is essential to us. At TCC, we strive to understand and meet the specific needs of our customers effectively.",
      target_fr:
        "La satisfaction du client est essentielle pour nous. À la TCC, nous nous efforçons de comprendre et de répondre efficacement aux besoins spécifiques de nos clients.",
      target_es:
        "La satisfacción del cliente es esencial para nosotros. En TCC, buscamos entender y satisfacer las necesidades específicas de nuestros clientes de manera efectiva.",
    },
    {
      source: "Precisão e Qualidade",
      target_en: "Precision and Quality",
      target_fr: "Précision et Qualité",
      target_es: "Precisión y Calidad",
    },
    {
      source:
        "A fabricação de moldes requer altos padrões de precisão e qualidade. Portanto, a cultura da TCC enfatiza o compromisso com a excelência e a busca pela perfeição em cada etapa do processo de produção.",
      target_en:
        "Mold manufacturing requires high standards of precision and quality. Therefore, TCC's culture emphasizes a commitment to excellence and the pursuit of perfection at every stage of the production process.",
      target_fr:
        "La fabrication de moules exige des normes élevées de précision et de qualité. Par conséquent, la culture de TCC met l'accent sur l'engagement envers l'excellence et la recherche de la perfection à chaque étape du processus de production.",
      target_es:
        "La fabricación de moldes requiere altos estándares de precisión y calidad. Por lo tanto, la cultura de TCC enfatiza el compromiso con la excelencia y la búsqueda de la perfección en cada etapa del proceso de producción.",
    },
    {
      source: "Adaptação e Agilidade",
      target_en: "Adaptation and Agility",
      target_fr: "Adaptation et Agilité",
      target_es: "Adaptación y Agilidad",
    },
    {
      source:
        "O mercado de fabricação está em constante evolução, com novas demandas e tecnologias emergentes. Portanto, a TCC adapta-se rapidamente às mudanças e responde de forma ágil às necessidades dos seus clientes.",
      target_en:
        "The manufacturing market is constantly evolving, with new demands and emerging technologies. Therefore, TCC adapts quickly to changes and responds agilely to the needs of its customers.",
      target_fr:
        "Le marché de la fabrication évolue constamment, avec de nouvelles demandes et technologies émergentes. Par conséquent, TCC s'adapte rapidement aux changements et répond de manière agile aux besoins de ses clients.",
      target_es:
        "El mercado de fabricación está en constante evolución, con nuevas demandas y tecnologías emergentes. Por lo tanto, TCC se adapta rápidamente a los cambios y responde ágilmente a las necesidades de sus clientes.",
    },
    {
      source: "Desenvolvimento profissional",
      target_en: "Professional Development",
      target_fr: "Développement professionnel",
      target_es: "Desarrollo profesional",
    },
    {
      source:
        "A TCC investe no desenvolvimento e crescimento profissional dos seus funcionários, oferece oportunidades de aprendizagem e desenvolvimento de habilidades para manter a sua equipa atualizada com as mais recentes tecnologias e práticas da indústria.",
      target_en:
        "TCC invests in the professional development and growth of its employees, providing learning opportunities and skill development to keep its team updated with the latest industry technologies and practices.",
      target_fr:
        "TCC investit dans le développement professionnel et la croissance de ses employés, offrant des opportunités d'apprentissage et de développement des compétences pour maintenir son équipe à jour avec les dernières technologies et pratiques de l'industrie.",
      target_es:
        "TCC invierte en el desarrollo y crecimiento profesional de sus empleados, proporcionando oportunidades de aprendizaje y desarrollo de habilidades para mantener a su equipo actualizado con las últimas tecnologías y prácticas de la industria.",
    },

    // Design, Protótipos e Pequenas Séries

    {
      source: "Design e Engenharia de novos produtos",
      target_en: "Design and Engineering of new products",
      target_fr: "Conception et ingénierie de nouveaux produits",
      target_es: "Diseño e Ingeniería de nuevos productos",
    },
    {
      source:
        "Podemos criar o seu molde de A a Z. A nossa equipa pode oferecer-lhe primeiro um design que corresponda às suas expectativas, combinando tecnologia e técnica.",
      target_en:
        "We can create your mold from A to Z. Our team can first offer you a design that meets your expectations, combining technology and technique.",
      target_fr:
        "Nous pouvons créer votre moule de A à Z. Notre équipe peut d'abord vous proposer un design qui répond à vos attentes, en combinant technologie et technique.",
      target_es:
        "Podemos crear su molde de la A a la Z. Nuestro equipo puede ofrecerle primero un diseño que cumpla con sus expectativas, combinando tecnología y técnica.",
    },
    {
      source: "1 - Research",
      target_en: "1 - Research",
      target_fr: "1 - Recherche",
      target_es: "1 - Investigación",
    },
    {
      source: "2 – Conceito de design",
      target_en: "2 - Design Concept",
      target_fr: "2 - Concept de design",
      target_es: "2 - Concepto de diseño",
    },
    {
      source: "3 – Desenvolvimento do projeto",
      target_en: "3 - Project Development",
      target_fr: "3 - Développement du projet",
      target_es: "3 - Desarrollo del proyecto",
    },
    {
      source: "4 – Engenharia",
      target_en: "4 - Engineering",
      target_fr: "4 - Ingénierie",
      target_es: "4 - Ingeniería",
    },
    {
      source: "5 – Prototipagem",
      target_en: "5 - Prototyping",
      target_fr: "5 - Prototypage",
      target_es: "5 - Prototipado",
    },
    {
      source: "6 – Pequenas séries",
      target_en: "6 - Small Series",
      target_fr: "6 - Petites séries",
      target_es: "6 - Pequeñas series",
    },
    {
      source: "Processo de Desenvolvimento de um novo produto",
      target_en: "New product development process",
      target_fr: "Processus de développement d'un nouveau produit",
      target_es: "Proceso de desarrollo de un nuevo producto",
    },
    {
      source: "Prototipagem e Produção Rápida",
      target_en: "Prototyping and Rapid Production",
      target_fr: "Prototypage et Production Rapide",
      target_es: "Prototipado y Producción Rápida",
    },
    {
      source:
        "Em seguida, criamos um protótipo do molde para verificar com o nosso cliente se tudo está como ele imaginou. Ao trabalharmos em estreita colaboração com os nossos clientes, asseguramos o seu envolvimento ativo em todas as fases, garantindo resultados mais rápidos e rigorosamente alinhados com os objectivos do projeto.",
      target_en:
        "Next, we create a mold prototype to verify with our client that everything is as they envisioned. By working closely with our clients, we ensure their active involvement in all phases, guaranteeing faster results that are strictly aligned with the project objectives.",
      target_fr:
        "Ensuite, nous créons un prototype de moule pour vérifier avec notre client que tout est conforme à ce qu'il avait imaginé. En travaillant en étroite collaboration avec nos clients, nous assurons leur implication active à toutes les étapes, garantissant des résultats plus rapides et rigoureusement alignés sur les objectifs du projet.",
      target_es:
        "A continuación, creamos un prototipo del molde para verificar con nuestro cliente que todo está como él imaginó. Al trabajar en estrecha colaboración con nuestros clientes, aseguramos su participación activa en todas las fases, garantizando resultados más rápidos y estrictamente alineados con los objetivos del proyecto.",
    },
    {
      source:
        "Produzimos as suas peças em pequenas e médias séries para garantir uma versão final perfeita.",
      target_en:
        "We produce your parts in small and medium series to ensure a perfect final version.",
      target_fr:
        "Nous produisons vos pièces en petites et moyennes séries pour garantir une version finale parfaite.",
      target_es:
        "Producimos sus piezas en series pequeñas y medianas para garantizar una versión final perfecta.",
    },
    {
      source: "Prototipagem",
      target_en: "Prototyping",
      target_fr: "Prototypage",
      target_es: "Prototipado",
    },
    {
      source:
        "A prototipagem testa e verifica rigorosamente o design do produto, garantindo uma estética, ergonomia e funcionalidade irrepreensíveis.",
      target_en:
        "Prototyping rigorously tests and verifies the product design, ensuring impeccable aesthetics, ergonomics, and functionality.",
      target_fr:
        "Le prototypage teste et vérifie rigoureusement le design du produit, garantissant une esthétique, une ergonomie et une fonctionnalité irréprochables.",
      target_es:
        "El prototipado prueba y verifica rigurosamente el diseño del producto, asegurando una estética, ergonomía y funcionalidad impecables.",
    },
    {
      source:
        "Este processo eficiente ajuda a identificar as necessidades de melhoria e a corrigi-las num curto espaço de tempo para reduzir o prazo de entrega.",
      target_en:
        "This efficient process helps identify improvement needs and correct them in a short time to reduce the delivery time.",
      target_fr:
        "Ce processus efficace aide à identifier les besoins d'amélioration et à les corriger en peu de temps afin de réduire le délai de livraison.",
      target_es:
        "Este proceso eficiente ayuda a identificar las necesidades de mejora y a corregirlas en un corto período de tiempo para reducir el plazo de entrega.",
    },
    {
      source: "Pequenas Séries",
      target_en: "Small Series",
      target_fr: "Pré-série",
      target_es: "Serie corta",
    },
    {
      source:
        "O fabrico de baixo volume é adequado para empresas que não necessitam de grandes quantidades. Garante uma entrega rápida e uma boa relação qualidade/preço. Esta etapa económica ajuda a evitar problemas futuros e valida todos os aspectos do produto final.",
      target_en:
        "Low-volume manufacturing is suitable for companies that do not require large quantities. It ensures fast delivery and a good quality/price ratio. This cost-effective step helps avoid future problems and validates all aspects of the final product.",
      target_fr:
        "La fabrication à faible volume convient aux entreprises qui n'ont pas besoin de grandes quantités. Elle garantit une livraison rapide et un bon rapport qualité/prix. Cette étape économique aide à éviter les problèmes futurs et valide tous les aspects du produit final.",
      target_es:
        "La fabricación de bajo volumen es adecuada para empresas que no necesitan grandes cantidades. Garantiza una entrega rápida y una buena relación calidad/precio. Este paso económico ayuda a evitar problemas futuros y valida todos los aspectos del producto final.",
    },

    // Produção de Moldes e Validação

    {
      source: "Moldes de Alta Precisão",
      target_en: "High Precision Molds",
      target_fr: "Moules de haute précision",
      target_es: "Moldes de alta precisión",
    },
    {
      source:
        "Operando em condições ambientais controladas, dispomos de tecnologias que nos permitem maquinar produtos ultra-precisos, quer se trate de moldes para a indústria médica ou automóvel, etc.",
      target_en:
        "Operating under controlled environmental conditions, we have technologies that allow us to machine ultra-precise products, whether it's molds for the medical or automotive industry, etc.",
      target_fr:
        "Opérant dans des conditions environnementales contrôlées, nous disposons de technologies nous permettant d'usiner des produits ultra-précis, que ce soit des moules pour l'industrie médicale ou automobile, etc.",
      target_es:
        "Operando en condiciones ambientales controladas, contamos con tecnologías que nos permiten mecanizar productos ultra precisos, ya sean moldes para la industria médica o automotriz, etc.",
    },
    {
      source: "1 - Orçamento",
      target_en: "1 - Budgeting",
      target_fr: "1 - Budget",
      target_es: "1 - Presupuesto",
    },
    {
      source: "2 – Conceção e Engenharia",
      target_en: "2 - Design and Engineering",
      target_fr: "2 - Conception et ingénierie",
      target_es: "2 - Diseño e Ingeniería",
    },
    {
      source: "3 – Moldes Protótipo",
      target_en: "3 - Prototype Molds",
      target_fr: "3 - Moules Prototype",
      target_es: "3 - Moldes Prototipo",
    },
    {
      source: "4 – Produção de Moldes finais",
      target_en: "4 - Final Mold Production",
      target_fr: "4 - Production de moules finaux",
      target_es: "4 - Producción de Moldes Finales",
    },
    {
      source: "5 – Teste e Validação",
      target_en: "5 - Testing and Validation",
      target_fr: "5 - Test et validation",
      target_es: "5 - Prueba y Validación",
    },
    {
      source: "Processo de Fabrico de Moldes",
      target_en: "Mold Manufacturing Process",
      target_fr: "Processus de Fabrication de Moules",
      target_es: "Proceso de Fabricación de Moldes",
    },
    {
      source: "Instalações e Tecnologia dedicadas",
      target_en: "Dedicated Facilities and Technology",
      target_fr: "Installations et technologie dédiées",
      target_es: "Instalaciones y tecnología dedicadas",
    },

    {
      source: "- Injeção 2K (bimatéria)",
      target_en: "2K Injection Moulding",
      target_es: "Moldeo por inyección de dos componentes",
      target_fr: "Moulage par injection à deux composants",
    },

    {
      source: "- Insertos Metálicos",
      target_en: "Metal Insert Moulding",
      target_es: "Insertos metálicos en el molde",
      target_fr: "Inserts métalliques dans le moule",
    },
    {
      source: "Engenharia inversa",
      target_en: "Reverse Engineering",
      target_es: "Ingeniería inversa",
      target_fr: "Ingénierie inverse",
    },
    {
      source: "- Engenharia IMA",
      target_en: "In-Moulding Assembly",
      target_es: "Inyección con ensamblaje",
      target_fr: "Injection avec assemblage",
    },
    {
      source: "Injeção assistida a gás",
      target_en: "Gas Assisted Injection Moulding",
      target_es: "Moldeo por inyección asistida por gas",
      target_fr: "Moulage par injection assistée au gaz",
    },
    {
      source: "Instalações e Tecnologia dedicadas",
      target_en: "Dedicated Facilities and Technology",
      target_es: "Instalaciones y Tecnología dedicadas",
      target_fr: "Installations et technologie dédiées",
    },

    {
      source:
        "Ferramentas e instalações dedicadas permitem-nos ter experiência em muitas áreas e melhorar a eficiência da nossa produção. Oferecemos soluções eficazes e personalizadas para cada um dos nossos clientes.",
      target_en:
        "Dedicated tools and facilities enable us to gain expertise in various areas and enhance the efficiency of our production. We provide effective and customized solutions for each of our clients.",
      target_es:
        "Las herramientas y las instalaciones dedicadas nos permiten adquirir experiencia en diversas áreas y mejorar la eficiencia de nuestra producción. Ofrecemos soluciones efectivas y personalizadas para cada uno de nuestros clientes.",
      target_fr:
        "Les outils et installations dédiés nous permettent d'acquérir une expertise dans divers domaines et d'améliorer l'efficacité de notre production. Nous proposons des solutions efficaces et personnalisées pour chacun de nos clients.",
    },

    // Injeção de Termoplásticos e Assemblagem

    {
      source: "Sala Limpa",
      target_en: "Clean Room",
      target_es: "Sala Limpia",
      target_fr: "Salle Blanche",
    },
    {
      source: "ISO 6",
      target_en: "ISO Class 6",
      target_es: "Clase ISO 6",
      target_fr: "Classe ISO 6",
    },
    {
      source:
        "A nossa sala limpa permite a realização de ensaios num ambiente com muito poucas partículas. Ao atualizar as nossas capacidades com uma sala limpa ISO 6, o nosso objetivo é servir a indústria médica e o fabrico de dispositivos médicos numa base mais consistente e visível.",
      target_en:
        "Our clean room allows testing in an environment with very few particles. By upgrading our capabilities with an ISO Class 6 clean room, our goal is to better serve the medical industry and medical device manufacturing on a more consistent and visible basis.",
      target_es:
        "Nuestra sala limpia permite realizar pruebas en un ambiente con muy pocas partículas. Al actualizar nuestras capacidades con una sala limpia Clase ISO 6, nuestro objetivo es servir mejor a la industria médica y a la fabricación de dispositivos médicos de manera más consistente y visible.",
      target_fr:
        "Notre salle blanche permet de réaliser des tests dans un environnement avec très peu de particules. En améliorant nos capacités avec une salle blanche de classe ISO 6, notre objectif est de mieux servir l'industrie médicale et la fabrication de dispositifs médicaux de manière plus cohérente et visible.",
    },
    {
      source: "Normas de Referência",
      target_en: "Reference Standards",
      target_es: "Normas de referencia",
      target_fr: "Normes de référence",
    },
    {
      source: "Tecnologias de Injeção de Termoplásticos",
      target_en: "Thermoplastic Injection Moulding Technologies",
      target_es: "Tecnologías de Moldeo por Inyección de Termoplásticos",
      target_fr: "Technologies de Moulage par Injection de Thermoplastiques",
    },
    {
      source: "Tecnologias de Injeção",
      target_en: "Injection Technologies",
      target_es: "Tecnologías de Inyección",
      target_fr: "Technologies d'Injection"
    },
    {
      source: "- Injeção 2K (bimatéria)",
      target_en: "- 2K Injection Moulding",
      target_es: "- Moldeo por inyección 2K",
      target_fr: "- Moulage par Injection 2K",
    },
    {
      source: "- Insertos Metálicos",
      target_en: "- Metal Insert Moulding",
      target_es: "- Insertos metálicos en molde",
      target_fr: "- Inserts Métalliques dans le Moule",
    },
    {
      source: "- Engenharia inversa",
      target_en: "- Reverse Engineering",
      target_es: "- Ingeniería inversa",
      target_fr: "- Ingénierie Inverse",
    },
    {
      source: "- Engenharia IMA",
      target_en: "- Injection with Assembly",
      target_es: "- Inyección con ensamblaje",
      target_fr: "- Injection avec Assemblage",
    },
    {
      source: "- Injeção assistida a gás",
      target_en: "- Gas Assisted Injection Moulding",
      target_es: "- Moldeo por inyección asistida con gas",
      target_fr: "- Moulage par Injection Assistée au Gaz",
    },
    {
      source: "As nossas instalações e Equipamentos",
      target_en: "Our Facilities and Equipment",
      target_es: "Nuestras instalaciones y equipos",
      target_fr: "Nos Installations et Équipements",
    },
    {
      source:
        "A nossa oficina está equipada com 6 centros de maquinagem de fresagem CNC, 2 centros de maquinagem EDM de afundamento, 1 centro de maquinagem EDM de fio e máquinas de perfuração e retificação. Estas ferramentas renovadas regularmente garantem a eficácia e a qualidade dos moldes produzidos.",
      target_en:
        "Our shopfloor is equipped with 6 CNC milling machining centers, 2 sinker EDM machining centers, 1 wire EDM machining center, and drilling and grinding machines. These tools are regularly upgraded to ensure the effectiveness and quality of the molds produced.",
      target_es:
        "Nuestro taller está equipado con 6 centros de mecanizado fresadora CNC, 2 centros de mecanizado de hundimiento EDM, 1 centro de mecanizado de hilo EDM y máquinas de taladrado y rectificado. Estas herramientas se renuevan regularmente para garantizar la eficacia y la calidad de los moldes producidos.",
      target_fr:
        "Notre atelier est équipé de 6 centres d'usinage de fraisage CNC, 2 centres d'usinage EDM par enfonçage, 1 centre d'usinage EDM par fil et des machines de perçage et de rectification. Ces outils sont régulièrement mis à jour pour garantir l'efficacité et la qualité des moules produits.",
    },
    {
      source: "Equipamentos de Metrologia e Controlo de Qualidade",
      target_en: "Metrology and Quality Control Equipment",
      target_es: "Equipos de Metrología y Control de Calidad",
      target_fr: "Équipements de Métrologie et de Contrôle Qualité",
    },
    {
      source: "CMM Controlo Dimensional",
      target_en: "CMM Dimensional Control",
      target_es: "Control dimensional de MMC",
      target_fr: "Contrôle dimensionnel CMM",
    },

    // Indústrias e Casos de Estudo

    {
      source: "Indústrias e Casos de Estudo",
      target_en: "Industries and Case Studies",
      target_es: "Industrias y Casos de Estudio",
      target_fr: "Industries et Études de Cas",
    },

    //Noticias

    {
      source:
        "Modernização do processo de Electroerosão com tecnologia de ponta",
      target_en:
        "Modernization of the EDM process with cutting-edge technology",
      target_es:
        "Modernización del proceso de electroerosión con tecnología de punta",
      target_fr:
        "Modernisation du processus d'électroérosion avec une technologie de pointe",
    },
    {
      source:
        "A TCC adquiriu recentemente um equipamento de electroerosão com a mais recente tecnologia da OPS-INGERSOLL, equipado com um controlador de última geração. A aquisição deste equipamento permite uma redução do tempo electroerosão e uma redução no desgaste de elétrodos permitindo ganhos significativos na produtividade da TCC.",
      target_en:
        "TCC recently acquired an EDM machine with the latest technology from OPS-INGERSOLL, equipped with a state-of-the-art controller. The acquisition of this equipment allows for a reduction in EDM time and electrode wear, resulting in significant productivity gains for TCC.",
      target_es:
        "TCC adquirió recientemente una máquina de electroerosión con la última tecnología de OPS-INGERSOLL, equipada con un controlador de última generación. La adquisición de este equipo permite una reducción en el tiempo de electroerosión y el desgaste de los electrodos, lo que resulta en importantes ganancias de productividad para TCC.",
      target_fr:
        "TCC a récemment acquis une machine d'électroérosion équipée de la dernière technologie d'OPS-INGERSOLL, dotée d'un contrôleur de pointe. L'acquisition de cet équipement permet de réduire le temps d'électroérosion et l'usure des électrodes, ce qui entraîne des gains de productivité significatifs pour TCC.",
    },
    {
      source: "Novo Sistema de Medição BLUM",
      target_en: "New BLUM Measurement System",
      target_es: "Nuevo Sistema de Medición BLUM",
      target_fr: "Nouveau système de mesure BLUM",
    },
    {
      source:
        "A TCC equipou mais um centro de maquinação CNC com o sistema de medição BLUM Measurement Protocol o que permite melhorias significativas no controlo de qualidade da maquinação.",
      target_en:
        "TCC has equipped another CNC machining center with the BLUM Measurement Protocol system, which allows for significant improvements in machining quality control.",
      target_es:
        "TCC ha equipado otro centro de mecanizado CNC con el sistema BLUM Measurement Protocol, lo que permite mejoras significativas en el control de calidad del mecanizado.",
      target_fr:
        "TCC a équipé un autre centre d'usinage CNC avec le système BLUM Measurement Protocol, ce qui permet des améliorations significatives dans le contrôle qualité de l'usinage.",
    },
     {
      source: "A TCC reforça a sua capacidade produtiva e tecnológica",
      target_en: "TCC is expanding its production and technological capabilities",
      target_es: "TCC refuerza su capacidad productiva y tecnológica",
      target_fr: "TCC renforce ses capacités de production et technologiques",
    },
    {
      source:
        "Estes novos equipamentos de última geração permitem à TCC executar projetos técnicos de maior complexidade, com elevados requisitos de precisão e qualidade superficial, alargando substancialmente o leque de soluções que coloca ao serviço dos seus clientes. A tecnologia de 5 eixos possibilita operações mais eficientes, maior rigor dimensional e redução significativa de intervenções manuais.",
      target_en:
        "This new state-of-the-art equipment enables TCC to undertake more complex technical projects with stringent requirements for precision and surface quality, substantially expanding the range of solutions it offers its customers. The 5-axis technology allows for more efficient operations, greater dimensional accuracy, and a significant reduction in manual intervention.",
      target_es:
        "Estos nuevos equipos de última generación permiten a TCC llevar a cabo proyectos técnicos de mayor complejidad, con elevados requisitos de precisión y calidad superficial, ampliando considerablemente la gama de soluciones que pone al servicio de sus clientes. La tecnología de 5 ejes permite realizar operaciones más eficientes, ofrece una mayor precisión dimensional y reduce significativamente las intervenciones manuales.",
      target_fr:
        "Ces nouveaux équipements de pointe permettent à TCC de mener à bien des projets techniques plus complexes, exigeant une grande précision et une excellente qualité de surface, élargissant ainsi considérablement la gamme de solutions qu'elle propose à ses clients. La technologie à 5 axes permet des opérations plus efficaces, une plus grande précision dimensionnelle et une réduction significative des interventions manuelles.",
    },
    {
  source:
    "Com este investimento, a TCC garante <strong>melhor qualidade final</strong>, <strong>redução dos tempos de execução</strong> e <strong>maior fiabilidade nos prazos de entrega</strong>, reforçando a sua competitividade nos mercados mais exigentes e a confiança dos seus parceiros internacionais.",
  target_en:
    "With this investment, TCC ensures <strong>higher final quality</strong>, <strong>shorter turnaround times</strong>, and <strong>more reliable delivery schedules</strong>, thereby strengthening its competitiveness in the most demanding markets and the trust of its international partners.",
  target_es:
    "Con esta inversión, TCC garantiza una <strong>mejor calidad final</strong>, una <strong>reducción de los tiempos de ejecución</strong> y una <strong>mayor fiabilidad en los plazos de entrega</strong>, lo que refuerza su competitividad en los mercados más exigentes y la confianza de sus socios internacionales.",
  target_fr:
    "Grâce à cet investissement, TCC garantit une <strong>meilleure qualité finale</strong>, une <strong>réduction des délais d'exécution</strong> et une <strong>plus grande fiabilité dans le respect des délais de livraison</strong>, renforçant ainsi sa compétitivité sur les marchés les plus exigeants et la confiance de ses partenaires internationaux.",
},
    {
      source:
        "A aposta contínua em equipamentos avançados reflete ocompromisso da TCC com a inovação, a melhoria contínua e a criação de valor sustentável para os seus clientes.",
      target_en:
        "TCC’s ongoing investment in advanced equipment reflects its commitment to innovation, continuous improvement, and the creation of sustainable value for its customers.",
      target_es:
        "La apuesta constante por equipos avanzados refleja el compromiso de TCC con la innovación, la mejora continua y la creación de valor sostenible para sus clientes.",
      target_fr:
        "L'investissement constant dans des équipements de pointe témoigne de l'engagement de TCC en faveur de l'innovation, de l'amélioration continue et de la création de valeur durable pour ses clients.",
    },

    //Contacto

    {
      source: "Dep. Administrativo",
      target_en: "Administrative Department",
      target_es: "Departamento Administrativo",
      target_fr: "Service administratif",
    },
    {
      source: "Dep. Comercial",
      target_en: "Sales Department",
      target_es: "Departamento Comercial",
      target_fr: "Service commercial",
    },
    {
      source: "Dep. Técnico",
      target_en: "Technical Department",
      target_es: "Departamento Técnico",
      target_fr: "Service technique",
    },
    {
      source: "Comércio Internacional, Lda",
      target_en: "International Trade, Ltd.",
      target_es: "Comercio Internacional, S.L.",
      target_fr: "Commerce International, S.A.",
    },
    {
      source: "Entre em contacto",
      target_en: "Contact Us",
      target_es: "Póngase en contacto",
      target_fr: "Contactez-nous",
    },
    {
      source: "Nome",
      target_en: "Name",
      target_es: "Nombre",
      target_fr: "Nom",
    },
    {
      source: "Email",
      target_en: "Email",
      target_es: "Correo electrónico",
      target_fr: "Email",
    },
    {
      source: "Assunto",
      target_en: "Subject",
      target_es: "Asunto",
      target_fr: "Objet",
    },
    {
      source: "Mensagem",
      target_en: "Message",
      target_es: "Mensaje",
      target_fr: "Message",
    },
    {
      source: "Enviar",
      target_en: "Send",
      target_es: "Enviar",
      target_fr: "Envoyer",
    },
    {
      source: "Junta-te a nós",
      target_en: "Join Us",
      target_es: "Únete a nosotros",
      target_fr: "Rejoignez-nous",
    },
    {
      source: "Designer de produto",
      target_en: "Product Designer",
      target_es: "Diseñador de productos",
      target_fr: "Designer de produit",
    },
    {
      source: "Envia o teu currículo",
      target_en: "Send your resume",
      target_es: "Envía tu currículum",
      target_fr: "Envoyez votre CV",
    },
    {
      source: "Escolher Ficheiro",
      target_en: "Choose File",
      target_es: "Seleccionar Archivo",
      target_fr: "Choisir un fichier",
    },
    {
      source: "Subscrever",
      target_en: "Subscribe",
      target_es: "Suscribirse",
      target_fr: "S'abonner",
    },
    {
      source: "Indústrias",
      target_en: "Industries",
      target_es: "Industrias",
      target_fr: "Industries",
    },
       {
      source: "A TCC é fundamental na nossa produção de moldes. A precisão dos produtos é excelente e o suporte técnico responde rapidamente às nossas necessidades.",
      target_en: "TCC is fundamental to our mold production. The precision of the products is excellent and the technical support responds quickly to our needs.",
      target_es: "TCC es fundamental para nuestra producción de moldes. La precisión de los productos es excelente y el soporte técnico responde rápidamente a nuestras necesidades.",
      target_fr: "TCC est fondamental pour notre production de moules. La précision des produits est excellente et le support technique répond rapidement à nos besoins.",
    },
     {
      source: "Trabalhar com a TCC tem sido uma experiência positiva. A eficiência nas operações internacionais e a assistência contínua proporcionam uma tranquilidade inestimável nos nossos processos logísticos.",
      target_en: "Working with TCC has been a positive experience. The efficiency of international operations and ongoing assistance provide invaluable peace of mind in our logistics processes.",
      target_es: "Trabajar con TCC ha sido una experiencia positiva. La eficiencia de las operaciones internacionales y la asistencia continua proporcionan una tranquilidad inestimable en nuestros procesos logísticos.",
      target_fr: "Travailler avec le TCC a été une expérience positive. L'efficacité des opérations internationales et l'assistance permanente apportent une tranquillité d'esprit inestimable à nos processus logistiques.",
    },
     {
      source: "A inovação tecnológica da TCC e a sua capacidade de atender a projetos complexos surpreenderam-nos. O atendimento ao cliente é exemplar, sempre disponível e prestável.",
      target_en: "TCC's technological innovation and ability to handle complex projects surprised us. Their customer service is exemplary, always available and helpful.",
      target_es: "La innovación tecnológica de TCC y su capacidad para gestionar proyectos complejos nos sorprendieron. Su servicio de atención al cliente es ejemplar, siempre disponible y servicial.",
      target_fr: "L'innovation technologique de TCC et sa capacité à gérer des projets complexes nous ont surpris. Leur service client est exemplaire, toujours disponible et serviable.",
    },
     {
      source: "Dispomos de Centros de Testes e Validação de Moldes para avaliar uma vasta gama de moldes e tecnologias de injeção, incluindo IML (In-Mold-Labelling), IMA (In-Mold-Assembly), injeção 2K e 3K, injeção assistida a gás e sobremoldagem com inserções metálicas.",
      target_en: "We have Mold Testing and Validation Centers to evaluate a wide range of molds and injection technologies, including IML (In-Mold-Labelling), IMA (In-Mold-Assembly), 2K and 3K injection, gas-assisted injection and overmolding with metal inserts.",
      target_es: "Disponemos de Centros de Pruebas y Validación de Moldes para evaluar una amplia gama de moldes y tecnologías de inyección, como IML (In-Mold-Labelling), IMA (In-Mold-Assembly), inyección 2K y 3K, inyección asistida por gas y sobremoldeo con insertos metálicos.",
      target_fr: "Nous disposons de centres d'essai et de validation des moules pour évaluer une large gamme de moules et de technologies d'injection, notamment l'IML (In-Mold-Labelling), l'IMA (In-Mold-Assembly), l'injection 2K et 3K, l'injection assistée par gaz et le surmoulage avec des inserts métalliques.",
    },
    {
      source: "Incorporamos uma cultura de qualidade nos nossos moldes, combinando a expertise dos nossos engenheiros com equipamentos de última geração, variando de 25T a 1600T.",
      target_en: "We incorporate a culture of quality into our molds, combining the expertise of our engineers with state-of-the-art equipment, ranging from 25T to 1600T.",
      target_es: "Incorporamos una cultura de calidad a nuestros moldes, combinando la experiencia de nuestros ingenieros con equipos de última generación, que van desde las 25T hasta las 1600T.",
      target_fr: "Nous intégrons une culture de la qualité dans nos moules, en combinant l'expertise de nos ingénieurs avec un équipement de pointe, allant de 25T à 1600T.",
    },
      {
      source: "Proporcionamos todas as condições necessárias para testar e validar os moldes dos nossos clientes num ambiente de produção real. Isto é essencial, pois alcançar um processo estável e repetível é fundamental para a qualificação de um molde.",
      target_en: "We provide all the necessary conditions to test and validate our customers' molds in a real production environment. This is essential, as achieving a stable and repeatable process is fundamental to the qualification of a mold.",
      target_es: "Proporcionamos todas las condiciones necesarias para probar y validar los moldes de nuestros clientes en un entorno de producción real. Esto es esencial, ya que conseguir un proceso estable y repetible es fundamental para la cualificación de un molde.",
      target_fr: "Nous fournissons toutes les conditions nécessaires pour tester et valider les moules de nos clients dans un environnement de production réel. C'est essentiel, car l'obtention d'un processus stable et reproductible est fondamentale pour la qualification d'un moule.",
    },
     {
      source: "Oferecemos soluções de engenharia com processos flexíveis e otimizados.",
      target_en: "We offer engineering solutions with flexible and optimized processes.",
      target_es: "Ofrecemos soluciones de ingeniería con procesos flexibles y optimizados.",
      target_fr: "Nous proposons des solutions d'ingénierie avec des processus flexibles et optimisés.",
    },
    {
      source: "O rigor dimensional é crucial na área de produção de moldes, especialmente para uma empresa de excelência como a TCC.",
      target_en: "Dimensional accuracy is crucial in the area of mold production, especially for a company of excellence like TCC.",
      target_es: "El rigor dimensional es crucial en el ámbito de la producción de moldes, especialmente para una empresa de excelencia como TCC.",
      target_fr: "La rigueur dimensionnelle est cruciale dans le domaine de la production de moules, en particulier pour une entreprise d'excellence comme TCC.",
    },
    {
      source: "A precisão na fabricação dos moldes garante que as peças produzidas atendam às especificações técnicas exigidas, assegurando a qualidade e a funcionalidade do produto final.",
      target_en: "Precision in the manufacture of molds ensures that the parts produced meet the required technical specifications, guaranteeing the quality and functionality of the final product.",
      target_es: "La precisión en la fabricación de moldes asegura que las piezas producidas cumplan las especificaciones técnicas requeridas, garantizando la calidad y funcionalidad del producto final.",
      target_fr: "La précision dans la fabrication des moules permet de s'assurer que les pièces produites répondent aux spécifications techniques requises, garantissant ainsi la qualité et la fonctionnalité du produit final.",
    },
    {
      source: "Um controle dimensional rigoroso minimiza variações e defeitos, reduzindo desperdícios e custos de produção. Além disso, moldes com dimensões exatas prolongam a vida útil dos equipamentos, proporcionando maior confiabilidade e eficiência no processo produtivo.",
      target_en: "Strict dimensional control minimizes variations and defects, reducing waste and production costs. In addition, molds with exact dimensions extend the useful life of the equipment, providing greater reliability and efficiency in the production process.",
      target_es: "Un estricto control dimensional minimiza las variaciones y los defectos, reduciendo los residuos y los costes de producción. Además, los moldes con dimensiones exactas alargan la vida útil de los equipos, proporcionando mayor fiabilidad y eficiencia en el proceso de producción.",
      target_fr: "Un contrôle dimensionnel strict minimise les variations et les défauts, réduisant ainsi les déchets et les coûts de production. En outre, les moules aux dimensions exactes prolongent la durée de vie utile de l'équipement, ce qui accroît la fiabilité et l'efficacité du processus de production.",
    },
    {
      source: "Na TCC, o compromisso com o rigor dimensional é a base para a entrega de soluções de alta qualidade, atendendo às expectativas dos clientes e consolidando a posição de liderança no mercado.4o",
      target_en: "At TCC, the commitment to dimensional accuracy is the basis for delivering high quality solutions, meeting customer expectations and consolidating the company's leading position in the market.4o",
      target_es: "En TCC, el compromiso con el rigor dimensional es la base para ofrecer soluciones de alta calidad, satisfacer las expectativas de los clientes y consolidar la posición de liderazgo de la empresa en el mercado.4ª edición",
      target_fr: "Chez TCC, l'engagement en faveur de la rigueur dimensionnelle est à la base de la fourniture de solutions de haute qualité, de la satisfaction des attentes des clients et de la consolidation de la position de leader de l'entreprise sur le marché.4ème édition",
    },
     {
      source: "O BLUM Measurement Protocol é uma tecnologia avançada projetada para garantir precisão e eficiência na medição de componentes durante o processo de maquinação CNC.",
      target_en: "The BLUM Measurement Protocol is an advanced technology designed to guarantee precision and efficiency in the measurement of components during the CNC machining process.",
      target_es: "El protocolo de medición BLUM es una tecnología avanzada diseñada para garantizar la precisión y la eficacia en la medición de componentes durante el proceso de mecanizado CNC.",
      target_fr: "Le protocole de mesure BLUM est une technologie avancée conçue pour garantir la précision et l'efficacité de la mesure des composants pendant le processus d'usinage CNC.",
    },
    {
      source: "Este protocolo utiliza sondas de alta precisão para monitorizar dimensões e detetar desvios em tempo real, permitindo ajustes imediatos na produção. Com o BLUM Measurement Protocol, a TCC melhora significativamente a qualidade dos seus produtos, reduzindo desperdícios e otimizando os tempos de ciclo, resultando numa produção mais confiável e económica.",
      target_en: "This protocol uses high-precision probes to monitor dimensions and detect deviations in real time, allowing for immediate adjustments in production. With the BLUM Measurement Protocol, TCC significantly improves the quality of its products, reducing waste and optimizing cycle times, resulting in more reliable and economical production.",
      target_es: "Este protocolo utiliza sondas de alta precisión para controlar las dimensiones y detectar desviaciones en tiempo real, lo que permite realizar ajustes inmediatos en la producción. Con el protocolo de medición BLUM, TCC mejora significativamente la calidad de sus productos, reduciendo los residuos y optimizando los tiempos de ciclo, lo que se traduce en una producción más fiable y económica.",
      target_fr: "Ce protocole utilise des sondes de haute précision pour contrôler les dimensions et détecter les écarts en temps réel, ce qui permet d'ajuster immédiatement la production. Grâce au protocole de mesure BLUM, TCC améliore considérablement la qualité de ses produits, en réduisant les déchets et en optimisant les temps de cycle, ce qui se traduit par une production plus fiable et plus économique.",
    },
    {
      source: "Na TCC, o controlo dimensional por Máquina de Medição por Coordenadas (CMM) desempenha um papel vital na produção de moldes de alta precisão.",
      target_en: "At TCC, dimensional control by Coordinate Measuring Machine (CMM) plays a vital role in the production of high-precision molds.",
      target_es: "En TCC, el control dimensional mediante máquinas de medición por coordenadas (MMC) desempeña un papel fundamental en la producción de moldes de alta precisión.",
      target_fr: "Chez TCC, le contrôle dimensionnel par machine à mesurer tridimensionnelle (MMT) joue un rôle essentiel dans la production de moules de haute précision.",
    },
    {
      source: "Ao utilizar tecnologia avançada, o CMM permite medições tridimensionais extremamente precisas, garantindo que cada molde produzido atenda rigorosamente às especificações exigidas.",
      target_en: "By using advanced technology, the CMM enables extremely precise three-dimensional measurements, ensuring that each mold produced strictly meets the required specifications.",
      target_es: "Gracias al uso de tecnología avanzada, la MMC permite realizar mediciones tridimensionales extremadamente precisas, garantizando que cada molde producido cumpla estrictamente las especificaciones requeridas.",
      target_fr: "Grâce à une technologie avancée, la MMT permet d'effectuer des mesures tridimensionnelles extrêmement précises, ce qui garantit que chaque moule produit répond strictement aux spécifications requises.",
    },
     {
      source: "Este nível de controle não só assegura a qualidade e a conformidade das peças finais, mas também otimiza os processos de fabricação, reduzindo retrabalhos e desperdícios.",
      target_en: "This level of control not only ensures the quality and conformity of the final parts, but also optimizes manufacturing processes, reducing rework and waste.",
      target_es: "Este nivel de control no sólo garantiza la calidad y conformidad de las piezas finales, sino que también optimiza los procesos de fabricación, reduciendo las repeticiones y los residuos.",
      target_fr: "Ce niveau de contrôle permet non seulement de garantir la qualité et la conformité des pièces finales, mais aussi d'optimiser les processus de fabrication, en réduisant les retouches et les déchets.",
    },
    {
      source: "A dedicação da TCC ao uso do CMM reflete seu compromisso com a excelência e a inovação na indústria de moldes.",
      target_en: "TCC's dedication to using the CMM reflects its commitment to excellence and innovation in the moldmaking industry.",
      target_es: "La dedicación de TCC al uso de la MMC refleja su compromiso con la excelencia y la innovación en la industria de fabricación de moldes.",
      target_fr: "La volonté de TCC d'utiliser la MMT reflète son engagement en faveur de l'excellence et de l'innovation dans l'industrie de la fabrication de moules.",
    },
    {
      source: "O caso de estudo do desenvolvimento de um copo de plástico, Long Drink, que substitui o vidro, mantendo a mesma qualidade e características, representa uma inovação significativa no campo dos materiais e design de produtos. A pesquisa focou-se na criação de um material plástico avançado, que oferece a transparência, resistência e sensação tátil semelhantes às do vidro, mas com a vantagem adicional de ser mais leve e menos suscetível a quebras. Esse copo de plástico de alta qualidade foi projetado para replicar a experiência de uso do vidro em diversas aplicações, como em restaurantes, bares e ambientes domésticos, garantindo a mesma elegância e funcionalidade. Além disso, a utilização deste novo material contribui para a sustentabilidade, reduzindo a necessidade de reciclagem de vidro e o desperdício associado a copos quebrados, ao mesmo tempo que proporciona uma opção segura e durável para o consumidor final.",
      target_en: "The case study of the development of a plastic Long Drink cup that replaces glass while maintaining the same quality and characteristics represents a significant innovation in the field of materials and product design. The research focused on creating an advanced plastic material that offers the transparency, resistance and tactile sensation similar to glass, but with the added advantage of being lighter and less susceptible to breakage. This high-quality plastic cup has been designed to replicate the experience of using glass in a variety of applications, such as restaurants, bars and domestic environments, while guaranteeing the same elegance and functionality. In addition, the use of this new material contributes to sustainability by reducing the need for glass recycling and the waste associated with broken glasses, while providing a safe and durable option for the end consumer.",
      target_es: "El estudio de caso del desarrollo de un vaso de plástico para Long Drink que sustituye al vidrio manteniendo la misma calidad y características representa una innovación significativa en el campo de los materiales y el diseño de productos. La investigación se centró en la creación de un material plástico avanzado que ofrece la transparencia, resistencia y sensación táctil similares al vidrio, pero con la ventaja añadida de ser más ligero y menos susceptible de romperse. Este vaso de plástico de alta calidad se ha diseñado para replicar la experiencia de usar vidrio en diversas aplicaciones, como restaurantes, bares y entornos domésticos, garantizando al mismo tiempo la misma elegancia y funcionalidad. Además, el uso de este nuevo material contribuye a la sostenibilidad al reducir la necesidad de reciclar el vidrio y los residuos asociados a los vasos rotos, al tiempo que ofrece una opción segura y duradera para el consumidor final.",
      target_fr: "L'étude de cas sur le développement d'un gobelet Long Drink en plastique qui remplace le verre tout en conservant la même qualité et les mêmes caractéristiques représente une innovation significative dans le domaine des matériaux et de la conception de produits. La recherche s'est concentrée sur la création d'un matériau plastique avancé qui offre une transparence, une résistance et une sensation tactile similaires à celles du verre, mais avec l'avantage supplémentaire d'être plus léger et moins susceptible de se briser. Ce gobelet en plastique de haute qualité a été conçu pour reproduire l'expérience du verre dans diverses applications, telles que les restaurants, les bars et les environnements domestiques, tout en garantissant la même élégance et la même fonctionnalité. En outre, l'utilisation de ce nouveau matériau contribue au développement durable en réduisant la nécessité de recycler le verre et les déchets associés aux verres cassés, tout en offrant une option sûre et durable au consommateur final.",
    },
    {
      source: "Caso de Estudo",
      target_en: "Case Study",
      target_es: "Caso de Estudio",
      target_fr: "Étude de Cas"
    },
    {
      source: "1. Design de um novo produto",
      target_en: "1. Designing a new product",
      target_es: "1. Diseñar un nuevo producto",
      target_fr: "1. Concevoir un nouveau produit",
    },
    {
      source: "Na fase de design do produto, a equipa de desenvolvimento focou-se na criação de um copo que replicasse a estética e a funcionalidade do vidro. Foram realizados estudos de ergonomia, análise de mercado e seleção de materiais para garantir que o copo de plástico mantivesse a mesma elegância e sensação tátil do vidro. A fase de design incluiu esboços, modelagem 3D e a consideração de fatores como transparência, resistência e peso do material.",
      target_en: "In the product design phase, the development team focused on creating a glass that replicated the aesthetics and functionality of glass. Ergonomic studies, market analysis and material selection were carried out to ensure that the plastic cup maintained the same elegance and tactile sensation as the glass. The design phase included sketches, 3D modeling and the consideration of factors such as transparency, resistance and weight of the material.",
      target_es: "En la fase de diseño del producto, el equipo de desarrollo se centró en crear un vaso que reprodujera la estética y la funcionalidad del vidrio. Se realizaron estudios ergonómicos, análisis de mercado y selección de materiales para garantizar que el vaso de plástico conservara la misma elegancia y sensación táctil que el de cristal. La fase de diseño incluyó bocetos, modelado en 3D y la consideración de factores como la transparencia, la resistencia y el peso del material.",
      target_fr: "Lors de la phase de conception du produit, l'équipe de développement s'est attachée à créer un gobelet qui reproduise l'esthétique et la fonctionnalité du verre. Des études ergonomiques, une analyse du marché et une sélection des matériaux ont été réalisées pour s'assurer que le gobelet en plastique conservait la même élégance et la même sensation tactile que le verre. La phase de conception comprenait des croquis, une modélisation en 3D et la prise en compte de facteurs tels que la transparence, la résistance et le poids du matériau.",
    },
    {
      source: "2. Prototipagem",
      target_en: "2. Prototype",
      target_es: "2. Creación de prototipos",
      target_fr: "2. Le prototypage",
    },
     {
      source: "Utilizando a impressão 3D, foram criados protótipos do copo para testar e validar o design proposto. Esta etapa permitiu à equipa avaliar a precisão dos detalhes, a ergonomia e a usabilidade do copo, além de identificar possíveis melhorias antes da produção em larga escala.",
      target_en: "Using 3D printing, prototypes of the cup were created to test and validate the proposed design. This stage allowed the team to assess the accuracy of the details, the ergonomics and usability of the cup, as well as identifying possible improvements before large-scale production.",
      target_es: "Mediante impresión 3D, se crearon prototipos de la taza para probar y validar el diseño propuesto. Esta fase permitió al equipo evaluar la precisión de los detalles, la ergonomía y la facilidad de uso de la taza, así como identificar posibles mejoras antes de la producción a gran escala.",
      target_fr: "Grâce à l'impression 3D, des prototypes du gobelet ont été créés pour tester et valider la conception proposée. Cette étape a permis à l'équipe d'évaluer la précision des détails, l'ergonomie et la facilité d'utilisation du gobelet, et d'identifier les améliorations possibles avant la production à grande échelle.",
    },
    {
      source: "3. Desenvolvimento do Molde",
      target_en: "3. Mold development",
      target_es: "3. Desarrollo de moldes",
      target_fr: "3. Développement du moule",
    },
     {
      source: "Após a aprovação do design final e dos protótipos, iniciou-se o desenvolvimento do molde de injeção, essencial para a produção em massa do copo de plástico. Esta etapa envolveu a criação de moldes precisos, feitos de aço, que permitiram a replicação exata do design aprovado. O desenvolvimento do molde garantiu a consistência na fabricação dos copos, assegurando que cada unidade produzida mantivesse as características desejadas.",
      target_en: "Once the final design and prototypes had been approved, work began on developing the injection mold, which is essential for the mass production of the plastic cup. This stage involved the creation of precise molds, made of steel, which allowed for the exact replication of the approved design. The development of the mold guaranteed consistency in the manufacture of the cups, ensuring that each unit produced retained the desired characteristics.",
      target_es: "Una vez aprobados el diseño final y los prototipos, se empezó a desarrollar el molde de inyección, esencial para la producción en serie del vaso de plástico. En esta fase se crearon moldes precisos, de acero, que permitían reproducir con exactitud el diseño aprobado. El desarrollo del molde garantizó la uniformidad en la fabricación de los vasos, asegurando que cada unidad producida mantuviera las características deseadas.",
      target_fr: "Une fois la conception finale et les prototypes approuvés, le travail de développement du moule d'injection, indispensable à la production en série du gobelet en plastique, a commencé. Cette étape a impliqué la création de moules précis, en acier, qui ont permis de reproduire exactement le modèle approuvé. La mise au point du moule a permis de garantir l'uniformité de la fabrication des gobelets, en veillant à ce que chaque unité produite conserve les caractéristiques souhaitées.",
    },
    {
      source: "Produto Final",
      target_en: "Final Product",
      target_es: "Producto final",
      target_fr: "Produit final",
    },
     {
      source: "Com o molde desenvolvido, a produção do copo de plástico começou em escala industrial. O produto final foi submetido a rigorosos testes de qualidade para confirmar que mantinha a transparência, resistência e sensação tátil semelhantes às do vidro. O copo foi então embalado e distribuído, oferecendo uma alternativa inovadora e sustentável ao vidro, mantendo a mesma elegância e funcionalidade que os consumidores esperam.",
      target_en: "With the mold developed, production of the plastic cup began on an industrial scale. The final product was subjected to rigorous quality tests to confirm that it maintained the same transparency, resistance and tactile sensation as glass. The cup was then packaged and distributed, offering an innovative and sustainable alternative to glass while maintaining the same elegance and functionality that consumers expect.",
      target_es: "Una vez desarrollado el molde, se inició la producción del vaso de plástico a escala industrial. El producto final se sometió a rigurosas pruebas de calidad para confirmar que mantenía la misma transparencia, resistencia y sensación táctil que el vidrio. A continuación, el vaso se envasó y distribuyó, ofreciendo una alternativa innovadora y sostenible al vidrio, pero manteniendo la misma elegancia y funcionalidad que esperan los consumidores.",
      target_fr: "Une fois le moule mis au point, la production du gobelet en plastique a commencé à l'échelle industrielle. Le produit final a été soumis à des tests de qualité rigoureux pour confirmer qu'il conservait la même transparence, la même résistance et la même sensation tactile que le verre. Le gobelet a ensuite été emballé et distribué, offrant une alternative innovante et durable au verre tout en conservant l'élégance et la fonctionnalité attendues par les consommateurs.",
    },
    {
      source: "Área Comercial (Alemão)",
      target_en: "Commercial Area (German)",
      target_es: "Área comercial (alemán)",
      target_fr: "Zone commerciale (allemand)",
    },
    {
      source: "<strong>Visão Geral:</strong>",
      target_en: "<strong>Overview:</strong>",
      target_es: "<strong>Resumen:</strong>",
      target_fr: "<strong>Vue d'ensemble :</strong>",
    },
    {
      source:"Estamos à procura de um talentoso Comercial com conhecimentos de língua alemã para se juntar à nossa equipa de vendas. Este papel é essencial para expandir a nossa presença no mercado e fortalecer as relações com os nossos clientes de língua alemã. O candidato ideal terá uma paixão por vendas, uma compreensão profunda das dinâmicas do mercado e habilidades excecionais de comunicação.",
      target_en: "We are looking for a tal  ented salesperson with German language skills to join our sales team. This role is essential for expanding our market presence and strengthening relationships with our German-speaking customers. The ideal candidate will have a passion for sales, a deep understanding of market dynamics and exceptional communication skills.",
      target_es: "Buscamos un asistente comercial con talento y conocimientos de alemán para unirse a nuestro equipo de ventas. Este puesto es esencial para ampliar nuestra presencia en el mercado y reforzar las relaciones con nuestros clientes de habla alemana. El candidato ideal tendrá pasión por las ventas, un profundo conocimiento de la dinámica del mercado y unas excepcionales dotes de comunicación.",
      target_fr: "Nous recherchons un(e) assistant(e) commercial(e) talentueux(se) ayant des compétences en allemand pour rejoindre notre équipe de vente. Ce rôle est essentiel pour développer notre présence sur le marché et renforcer les relations avec nos clients germanophones. Le candidat idéal aura une passion pour la vente, une compréhension approfondie de la dynamique du marché et des compétences exceptionnelles en matière de communication.",
    },
    {
      source: "<strong>Responsabilidades:</strong>",
      target_en: "<strong>Responsibilities:</strong>",
      target_es: "<strong>Responsabilidades:</strong>",
      target_fr: "<strong>Responsabilités :</strong>",
    },
    {
      source: "Desenvolver e manter relações fortes com clientes e parceiros de língua alemã.",
      target_en: "Develop and maintain strong relationships with German-speaking customers and partners.",
      target_es: "Desarrollar y mantener relaciones sólidas con clientes y socios de habla alemana.",
      target_fr: "Développer et entretenir des relations solides avec les clients et partenaires germanophones.",
    },
     {
      source: "Identificar e explorar novas oportunidades de negócios no mercado de língua alemã.",
      target_en: "Identify and exploit new business opportunities in the German-speaking market.",
      target_es: "Identificar y explotar nuevas oportunidades de negocio en el mercado de habla alemana.",
      target_fr: "Identifier et exploiter de nouvelles opportunités commerciales sur le marché germanophone.",
    },
    {
      source: "Apresentar e promover os nossos produtos e serviços de forma eficaz, tanto presencialmente como virtualmente.",
      target_en: "To present and promote our products and services effectively, both in person and virtually.",
      target_es: "Presentar y promocionar nuestros productos y servicios de forma eficaz, tanto en persona como virtualmente.",
      target_fr: "Présenter et promouvoir nos produits et services de manière efficace, en personne et virtuellement.",
    },
    {
      source: "Negociar contratos e fechar vendas de forma eficiente, garantindo a satisfação do cliente.",
      target_en: "Negotiating contracts and closing sales efficiently, ensuring customer satisfaction.",
      target_es: "Negociar contratos y cerrar ventas con eficacia, garantizando la satisfacción del cliente.",
      target_fr: "Négocier des contrats et conclure des ventes de manière efficace, en veillant à la satisfaction du client.",
    },
      {
      source: "Colaborar com equipas internas para garantir a entrega de soluções que atendam às necessidades dos clientes.",
      target_en: "Collaborate with internal teams to ensure the delivery of solutions that meet customer needs.",
      target_es: "Colaborar con los equipos internos para garantizar la entrega de soluciones que satisfagan las necesidades de los clientes.",
      target_fr: "Collaborer avec les équipes internes pour garantir la fourniture de solutions répondant aux besoins des clients.",
    },
     {
      source: "Acompanhar tendências de mercado e feedback dos clientes para informar estratégias de vendas.",
      target_en: "Monitoring market trends and customer feedback to inform sales strategies.",
      target_es: "Supervisar las tendencias del mercado y los comentarios de los clientes para fundamentar las estrategias de venta.",
      target_fr: "Suivre les tendances du marché et le retour d'information des clients afin d'éclairer les stratégies de vente.",
    },
    {
      source: "Participar em feiras, conferências e eventos do setor para representar a empresa e gerar novos leads.‍",
      target_en: "Attend trade fairs, conferences and industry events to represent the company and generate new leads.‍",
      target_es: "Asistir a ferias, conferencias y eventos del sector para representar a la empresa y generar nuevos contactos.",
      target_fr: "Participer à des foires commerciales, des conférences et des événements industriels pour représenter l'entreprise et générer de nouveaux clients potentiels.‍",
    },
    {
      source: "<strong>Requisitos:</strong>",
      target_en: "<strong>Requirements:</strong>",
      target_es: "<strong>Requisitos:</strong>",
      target_fr: "<strong>Exigences :</strong>",
    },
      {
      source: "Experiência comprovada como Comercial ou em um papel similar.",
      target_en: "Proven experience as a sales assistant or in a similar role.",
      target_es: "Experiencia demostrada como asistente de ventas o en un puesto similar.",
      target_fr: "Expérience confirmée en tant qu'assistant commercial ou dans une fonction similaire.",
    },
    {
      source: "Fluência em alemão e português, tanto falado quanto escrito.",
      target_en: "Fluency in German and Portuguese, both spoken and written.",
      target_es: "Dominio del alemán y el portugués, tanto hablado como escrito.",
      target_fr: "Maîtrise de l'allemand et du portugais, tant à l'oral qu'à l'écrit.",
    },
     {
      source: "Excelente capacidade de comunicação e de construção de relações interpessoais.",
      target_en: "Excellent communication and interpersonal skills.",
      target_es: "Excelentes dotes de comunicación e interpersonales.",
      target_fr: "Excellentes compétences en matière de communication et de relations interpersonnelles.",
    },
    {
      source: "Habilidade em negociar e fechar vendas de forma eficaz.",
      target_en: "Ability to negotiate and close sales effectively.",
      target_es: "Capacidad para negociar y cerrar ventas con eficacia.",
      target_fr: "Capacité à négocier et à conclure des ventes de manière efficace.",
    },
    {
      source: "Conhecimento profundo das dinâmicas do mercado de vendas e técnicas de vendas modernas.",
      target_en: "In-depth knowledge of the dynamics of the sales market and modern sales techniques.",
      target_es: "Conocimiento profundo de la dinámica del mercado de ventas y de las técnicas de venta modernas.",
      target_fr: "Connaissance approfondie de la dynamique du marché de la vente et des techniques de vente modernes.",
    },
    {
      source: "Capacidade de trabalhar de forma autónoma e em equipa, num ambiente acelerado.",
      target_en: "Ability to work autonomously and as part of a team in a fast-paced environment.",
      target_es: "Capacidad para trabajar de forma autónoma y como parte de un equipo en un entorno de ritmo rápido.",
      target_fr: "Capacité à travailler de manière autonome et au sein d'une équipe dans un environnement en constante évolution.",
    },
    {
      source: "Familiaridade com ferramentas de CRM e software de vendas.‍",
      target_en: "Familiarity with CRM tools and sales software.‍",
      target_es: "Familiaridad con herramientas CRM y software de ventas.",
      target_fr: "Familiarité avec les outils de gestion de la relation client et les logiciels de vente.‍",
    },
     {
      source: "<strong>Educação e Experiência:</strong>",
      target_en: "<strong>Education and Experience:</strong>",
      target_es: "<strong>Formación y experiencia:</strong>",
      target_fr: "<strong>Formation et expérience :</strong>",
    },
     {
      source: "Licenciatura em Gestão, Marketing, Comércio Internacional ou área relacionada.",
      target_en: "Degree in Management, Marketing, International Trade or related field.",
      target_es: "Licenciatura en Gestión, Marketing, Comercio Internacional o campo relacionado.",
      target_fr: "Diplôme en gestion, marketing, commerce international ou dans un domaine connexe.",
    },
     {
      source: "Experiência prévia em vendas internacionais ou no mercado de língua alemã é uma vantagem.‍",
      target_en: "Previous experience in international sales or the German-speaking market is an advantage.‍",
      target_es: "Se valorará la experiencia previa en ventas internacionales o en el mercado de habla alemana.",
      target_fr: "Une expérience préalable dans la vente internationale ou sur le marché germanophone est un avantage.‍",
    },
      {
      source: "<strong>Benefícios:</strong>",
      target_en: "<strong>Benefits:</strong>",
      target_es: "<strong>Ventajas:</strong>",
      target_fr: "<strong>Avantages :</strong>",
    },
    {
      source: "Salário competitivo",
      target_en: "Competitive salary",
      target_es: "Salario competitivo",
      target_fr: "Salaire compétitif",
    },
    {
      source: "Ambiente de trabalho colaborativo e inovador",
      target_en: "Collaborative and innovative work environment",
      target_es: "Entorno de trabajo colaborativo e innovador",
      target_fr: "Environnement de travail collaboratif et innovant",
    },
    {
      source: "Oportunidades de crescimento profissional",
      target_en: "Opportunities for professional growth",
      target_es: "Oportunidades de crecimiento profesional",
      target_fr: "Possibilités de développement professionnel",
    },
    {
      source: "Gestor(a) da Qualidade",
      target_en: "Quality Manager",
      target_es: "Responsable de Calidad",
      target_fr: "Responsable Qualité",
    },
    {
      source: "Na TCC – Comércio Internacional, Lda., empresa fundada em 1993 e especializada na elaboração de projetos, fabrico e controlo de qualidade de moldes de injeção para a indústria dos plásticos, procuramos reforçar a nossa equipa com um(a) <strong>Gestor(a) da Qualidade</strong>, para assegurar a gestão da qualidade e do sistema de controloda organização.",
      target_en: "At TCC – Comércio Internacional, Lda., a company founded in 1993 and specializing in the design, manufacture, and quality control of injection molds for the plastics industry, we are looking to strengthen our team with a <strong>Quality Manager</strong> to oversee the organization’s quality management and control systems.",
      target_es: "En TCC – Comércio Internacional, Lda., empresa fundada en 1993 y especializada en el diseño, la fabricación y el control de calidad de moldes de inyección para la industria del plástico, buscamos reforzar nuestro equipo con un/a <strong>Responsable de Calidad</strong>, para garantizar la gestión de la calidad y del sistema de control de la organización.",
      target_fr: "Chez TCC – Comércio Internacional, Lda., entreprise fondée en 1993 et spécialisée dans la conception, la fabrication et le contrôle qualité de moules d'injection pour l'industrie des plastiques, nous cherchons à renforcer notre équipe avec un(e) <strong>Responsable Qualité</strong>, chargé(e) d'assurer la gestion de la qualité et du système de contrôle de l'organisation.",
    },
    {
      source: "<strong>Principais responsabilidades</strong>",
      target_en: "<strong>Key responsibilities</strong>",
      target_es: "<strong>Responsabilidades principales</strong>",
      target_fr: "<strong>Principales responsabilités</strong>",
    },
    {
      source: "Gerir, manter e melhorar o <strong>Sistema de Gestão da Qualidade</strong>;",
      target_en: "Manage, maintain, and improve the <strong>Quality Management System</strong>;",
      target_es: "Gestionar, mantener y mejorar el <strong>Sistema de Gestión de la Calidad</strong>;",
      target_fr: "Gérer, maintenir et améliorer le <strong>Système de Gestion de la Qualité</strong>;",
    },
{
  source: "Garantir o cumprimento dos requisitos da <strong>ISO9001:2015</strong> e dos procedimentos internos;",
  target_en: "Ensure compliance with the requirements of <strong>ISO9001:2015</strong> and internal procedures;",
  target_es: "Garantizar el cumplimiento de los requisitos de la norma <strong>ISO9001:2015</strong> y de los procedimientos internos;",
  target_fr: "Veiller au respect des exigences de la norme <strong>ISO9001:2015</strong> et des procédures internes;",
},
    {
      source: "Planear e acompanhar auditorias internas e externas;",
      target_en: "Plan and monitor internal and external audits;",
      target_es: "Planificar y supervisar auditorías internas y externas;",
      target_fr: "Planifier et suivre les audits internes et externes;",
    },
    {
      source: "Monitorizar indicadores de qualidade e promover ações de melhoria contínua;",
      target_en: "Monitor quality indicators and promote continuous improvement initiatives;",
      target_es: "Supervisar los indicadores de calidad y promover medidas de mejora continua;",
      target_fr: "Suivre les indicateurs de qualité et promouvoir des mesures d'amélioration continue;",
    },
    {
      source: "Assegurar o controlo documental associado ao sistema da qualidade;",
      target_en: "Ensure document control related to the quality system;",
      target_es: "Garantizar el control documental relacionado con el sistema de calidad;",
      target_fr: "Assurer le contrôle documentaire lié au système qualité;",
    },
     {
      source: "Tratar não conformidades, implementar ações corretivas e preventivas e acompanhar a sua eficácia;",
      target_en: "Address nonconformities, implement corrective and preventive actions, and monitor their effectiveness;",
      target_es: "Gestionar los incumplimientos, aplicar medidas correctivas y preventivas y supervisar su eficacia;",
      target_fr: "Traiter les non-conformités, mettre en œuvre des mesures correctives et préventives et en suivre l'efficacité;",
    },
    {
      source: "Apoiar os diferentes departamentos na melhoria de processos e na consolidação de uma cultura de qualidade.",
      target_en: "Support the various departments in improving processes and fostering a culture of quality.",
      target_es: "Apoyar a los distintos departamentos en la mejora de los procesos y en la consolidación de una cultura de calidad.;",
      target_fr: "Accompagner les différents services dans l'amélioration des processus et la mise en place d'une culture de la qualité.",
    },
{ source: "<strong>Perfil pretendido</strong>",
 target_en: "<strong>Desired profile</strong>",
 target_es: "<strong>Perfil deseado</strong>",
 target_fr: "<strong>Profil recherché</strong>",
},
{
  source: "Formação superior em Engenharia, Gestão Industrial, Qualidade ou área similar;",
  target_en: "Higher education degree in Engineering, Industrial Management, Quality, or a related field;",
  target_es: "Formación superior en Ingeniería, Gestión Industrial, Calidad o área similar;",
  target_fr: "Diplôme de l'enseignement supérieur en Ingénierie, Gestion Industrielle, Qualité ou domaine similaire;",
},
{
  source: "Experiência em funções ligadas à gestão da qualidade em contexto industrial;",
  target_en: "Experience in quality management roles within an industrial environment;",
  target_es: "Experiencia en funciones relacionadas con la gestión de la calidad en un entorno industrial;",
  target_fr: "Expérience dans des fonctions liées à la gestion de la qualité en environnement industriel;",
},
{ source: "Conhecimentos sólidos de sistemas de gestão da qualidade e da norma <strong>ISO9001</strong>;",
 target_en: "Strong knowledge of quality management systems and the <strong>ISO9001</strong> standard;",
 target_es: "Conocimientos sólidos de sistemas de gestión de la calidad y de la norma <strong>ISO9001</strong>;",
 target_fr: "Solides connaissances des systèmes de gestion de la qualité et de la norme <strong>ISO9001</strong>;",
},
{
  source: "Capacidade de análise, organização e orientação para a melhoria contínua;",
  target_en: "Strong analytical and organizational skills, with a focus on continuous improvement;",
  target_es: "Capacidad de análisis, organización y orientación hacia la mejora continua;",
  target_fr: "Capacité d'analyse, d'organisation et orientation vers l'amélioration continue ;",
},
{
  source: "Boa capacidade de comunicação e trabalho em equipa;",
  target_en: "Good communication skills and ability to work in a team;",
  target_es: "Buenas habilidades de comunicación y trabajo en equipo;",
  target_fr: "Bonnes compétences en communication et esprit d'équipe ;",
},
{
  source: "Sentido de responsabilidade, autonomia e rigor;",
  target_en: "Sense of responsibility, autonomy, and attention to detail;",
  target_es: "Sentido de la responsabilidad, autonomía y rigor;",
  target_fr: "Sens des responsabilités, autonomie et rigueur ;",
},
{
  source: "Valorizamos experiência no setor dos moldes, metalomecânica ou indústria transformadora.",
  target_en: "Experience in the mould, metalworking, or manufacturing industries will be valued.",
  target_es: "Se valorará la experiencia en el sector de moldes, metalmecánico o industria manufacturera.",
  target_fr: "Une expérience dans le secteur des moules, de la métallurgie ou de l'industrie manufacturière sera appréciée.",
},
{
  source: "<strong>O que oferecemos</strong>",
  target_en: "<strong>What we offer</strong>",
  target_es: "<strong>Lo que ofrecemos</strong>",
  target_fr: "<strong>Ce que nous offrons</strong>",
},
{
  source: "Integração numa empresa sólida e reconhecida no setor;",
  target_en: "Integration into a solid and well-established company in the sector;",
  target_es: "Incorporación a una empresa sólida y reconocida en el sector;",
  target_fr: "Intégration dans une entreprise solide et reconnue dans le secteur ;",
},
{
  source: "Participação ativa na melhoria e evolução dos processos internos;",
  target_en: "Active involvement in the improvement and development of internal processes;",
  target_es: "Participación activa en la mejora y evolución de los procesos internos;",
  target_fr: "Participation active à l'amélioration et à l'évolution des processus internes ;",
},
{
  source: "Envolvimento em projetos desafiantes num ambiente industrial exigente;",
  target_en: "Involvement in challenging projects within a demanding industrial environment;",
  target_es: "Participación en proyectos desafiantes en un entorno industrial exigente;",
  target_fr: "Participation à des projets stimulants dans un environnement industriel exigeant ;",
},
{
  source: "Oportunidade de crescimento profissional numa organização orientada para a qualidade e inovação.",
  target_en: "Opportunity for professional growth in an organization focused on quality and innovation.",
  target_es: "Oportunidad de crecimiento profesional en una organización orientada a la calidad y la innovación.",
  target_fr: "Possibilité d'évolution professionnelle au sein d'une organisation tournée vers la qualité et l'innovation.",
},
{
  source: "Se procuras um novo desafio profissional e queres integrar uma empresa com forte enfoque na excelência e melhoria contínua, envia-nos a tua candidatura.",
  target_en: "If you are looking for a new professional challenge and would like to join a company strongly focused on excellence and continuous improvement, send us your application.",
  target_es: "Si buscas un nuevo reto profesional y deseas formar parte de una empresa con un fuerte enfoque en la excelencia y la mejora continua, envíanos tu candidatura.",
  target_fr: "Si vous recherchez un nouveau défi professionnel et souhaitez rejoindre une entreprise fortement axée sur l'excellence et l'amélioration continue, envoyez-nous votre candidature.",
},
{
  source: "Candidaturas:",
  target_en: "Applications:",
  target_es: "Candidaturas:",
  target_fr: "Candidatures:",
},
{
  source: "TCC – Comércio Internacional, Lda. – info@tcc.pt",
  target_en: "TCC – Comércio Internacional, Lda. – info@tcc.pt",
  target_es: "TCC – Comércio Internacional, Lda. – info@tcc.pt",
  target_fr: "TCC – Comércio Internacional, Lda. – info@tcc.pt",
},
    {
      source: "Candidate-se a esta posição",
      target_en: "Apply for this position",
      target_es: "Solicitar este puesto",
      target_fr: "Postuler à ce poste",
    },
     {
      source: "Nome Completo",
      target_en: "Full Name",
      target_es: "Nombre y apellidoso",
      target_fr: "Nom Complet",
    },
    {
      source: "Numero para contacto",
      target_en: "Contact number",
      target_es: "Número de contacto",
      target_fr: "Numéro de contact",
    },
    {
      source: "Os nossos serviços na área do desenvolvimento de produto",
      target_en: "Our product development services",
      target_es: "Nuestros servicios de desarrollo de productos",
      target_fr: "Nos services de développement de produits",
    },
    {
      source: "Design e Engenharia de um novo produto",
      target_en: "Design and engineering of a new product",
      target_es: "Diseño e ingeniería de un nuevo productos",
      target_fr: "Conception et ingénierie d'un nouveau produit",
    },
      {
      source: "- Desenvolvimento de produto",
      target_en: "- Product development",
      target_es: "- Desarrollo de productos",
      target_fr: "- Développement de produits",
    },
    {
      source: "- Design de produto",
      target_en: "- Product design",
      target_es: "- Diseño del producto",
      target_fr: "- Conception du produit",
    },
    {
      source: "- Engenharia",
      target_en: "- Engineering",
      target_es: "- Ingeniería",
      target_fr: "- Ingénierie",
    },
    {
      source: "- Desenho CAD 3D",
      target_en: "- 3D CAD design",
      target_es: "- Dibujo CAD 3D",
      target_fr: "- Dessin CAD en 3D",
    },
     {
      source: "- Protótipos de validação",
      target_en: "- Prototypes de validation",
      target_es: "- Prototipos de validación",
      target_fr: "- Prototypes de validation",
    },
     {
      source: "- Engenharia inversa",
      target_en: "- Reverse engineering",
      target_es: "- Ingeniería inversa",
      target_fr: "- Ingénierie inverse",
    },
     {
      source: "Prototipagem e Produção Rápida",
      target_en: "Rapid Prototyping and Production",
      target_es: "Creación rápida de prototipos y producción",
      target_fr: "Prototypage et production rapides",
    },
    {
      source: "- Protótipos funcionais",
      target_en: "- Functional prototypes",
      target_es: "- Prototipos funcionales",
      target_fr: "- Prototypes fonctionnels",
    },
     {
      source: "- Pequenas séries",
      target_en: "- Small series",
      target_es: "- Series cortas",
      target_fr: "- Petites séries",
    },
  ],
};
